...
public class GatewayAggregateCommandResource {
	 ...
	 *         loveDTO, or with status 400 (Bad Request) if the love has already an
	 *         ID
	...
	public ResponseEntity<LoveDTO> loveCommittedActivity(@RequestBody LoveDTO loveDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, result.toString()))
				.build();
	}
	...
	public ResponseEntity<Void> unloveCommittedActivity(@RequestBody DeleteLoveModel deleteLoveModel) {
		...
		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, deleteLoveModel.toString())).build();
	}
	...
	public ResponseEntity<CommentDTO> saveComment(@RequestBody CommentDTO commentDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, commentDto.toString()))
				.build();
	}
	 ...
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         replyDTO, or with status 400 (Bad Request) if the reply has already
	 *         an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	...
	public ResponseEntity<ReplyDTO> saveReply(@RequestBody ReplyDTO replyDTO) throws URISyntaxException {
		...
		return ResponseEntity.ok().headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, replyDto.toString()))
				.build();
	}
	...
	@Timed
	public ResponseEntity<ActivityAggregate> createActivity(@RequestBody ActivityAggregate activityAggregate)
		...
		/*
		 * ActivityAggregate result =
		 * aggregateCommandResourceApi.createActivityUsingPOST1(activityAggregate).
		 * getBody();
		 * 
		 * return
	...
	@Timed
	public ResponseEntity<ActivityAggregate> updateActivity(@RequestBody ActivityAggregate activityAggregate)
			throws URISyntaxException {
	 ...
	 * @return the ResponseEntity with status 201 (Created) and with body the new
	 *         committedActivityDTO, or with status 400 (Bad Request) if the
	 *         committedActivity has already an ID
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PostMapping("/create-committed-activity")
	@Timed
	public ResponseEntity<CommittedActivityDTO> createCommittedActivity(
			@RequestBody CommittedActivityDTO committedActivityDTO) throws URISyntaxException {
		log.debug("REST request to save CommittedActivity : {}", committedActivityDTO);
		if (committedActivityDTO.getId() != null) {
			throw new BadRequestAlertException("A new committedActivity cannot already have an ID", ENTITY_NAME,
					"idexists");
		}

		CommittedActivityDTO committedActivityDto = aggregateCommandResourceApi
				.createCommittedActivityUsingPOST(committedActivityDTO).getBody();

		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, committedActivityDto.toString())).build();
	}
	 ...
	 * @return the ResponseEntity with status 200 (OK) and with body the updated
	 *         committedActivityDTO, or with status 400 (Bad Request) if the
	 *         committedActivityDTO is not valid, or with status 500 (Internal
	 *         Server Error) if the committedActivityDTO couldn't be updated
	 * @throws URISyntaxException if the Location URI syntax is incorrect
	 */
	@PutMapping("/update-committed-activity")
	@Timed
	public ResponseEntity<CommittedActivityDTO> updateCommittedActivity(
			@RequestBody CommittedActivityDTO committedActivityDTO) throws URISyntaxException {
		...
		CommittedActivityDTO committedActivityDto = aggregateCommandResourceApi
				.updateCommittedActivityUsingPUT(committedActivityDTO).getBody();
	...
	public ResponseEntity<RegisteredUserDTO> createRegisteredUser(@RequestBody RegisteredUserDTO registeredUserDTO)
		...
		RegisteredUserDTO registeredUserDto = aggregateCommandResourceApi
				.createRegisteredUserUsingPOST(registeredUserDTO).getBody();

		return ResponseEntity.ok()
				.headers(HeaderUtil.createEntityDeletionAlert(ENTITY_NAME, registeredUserDto.toString())).build();
	...
	public ResponseEntity<RegisteredUserDTO> updateRegisteredUser(@RequestBody RegisteredUserDTO registeredUserDTO)
		...
		RegisteredUserDTO registeredUserDto = aggregateCommandResourceApi
				.updateRegisteredUserUsingPUT(registeredUserDTO).getBody();
